# Instruções para GitHub Copilot — Forja
...
> Guia para o Copilot operar a Forja. O conteúdo é o mesmo para Claude, Gemini e Codex — a Forja é multi-IA por design.
...
2. **Processo que governa** — features usam specs proporcionais ao escopo; nada estrutural sem ADR; handoffs de 7 campos (ADR-0005); comandos auditados pelo core (ADR-0020).
...
node bin/forja.ts <comando>    # workspace, spec, gsd, code, memória, contexto
```

O core aplica **gates** (ex.: comandos de produto falham cedo sem workspace) e grava **auditoria** de cada execução em `<workspace>/.context/forja-runs.jsonl`. Os scripts npm são aliases finos do core. Comando novo = entrada em `lib/core/registry.ts` (validado por `test/forja-core.test.js`).
...
2. `npm run context:smart` — pack mínimo-suficiente no modo certo (`task` para uma feature, `domain` para uma área, `global` só para onboarding). Meça a economia na tarefa atual.
3. Use resumos existentes quando forem atuais; confira a fonte quando faltarem evidências.
