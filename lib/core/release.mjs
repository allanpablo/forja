...
export async function withCleanInstall(fn, { root = repoRoot } = {}) {
  ...
  // Sem NODE_PATH e sem npm_config_* herdados: o processo filho não pode enxergar o repo.
  ...
  try {
    ...
    execFileSync('npm', ['init', '-y'], { cwd: dir, encoding: 'utf8', env, stdio: 'ignore' });
    execFileSync('npm', ['install', path.join(dir, tgz)], {
      cwd: dir,
      encoding: 'utf8',
      env,
      stdio: 'ignore',
...
const registryScripts = {
  ...
  probe(env) {
    ...
    for (const [name, def] of Object.entries(env.registry)) {
      ...
      if (!rel) continue;
      total += 1;
      if (!env.fs.existsSync(path.join(env.pkgDir, rel))) faltando.push(`${name} → ${rel}`);
    }
...
function publishedSources(env) {
  ...
  for (const dir of CODE_DIRS) {
    const full = path.join(env.pkgDir, dir);
...
const smokeCommands = {
  ...
  dependsOn: 'install',
  probe(env) {
    const bin = path.join(env.pkgDir, 'bin', 'forja.mjs');
    const quebrados = [];
    ...
    for (const args of env.smokeCommands) {
      ...
      if (LOADER_ERRORS.test(saida)) {
        const causa = saida.match(LOADER_ERRORS)[0];
